...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    } else if (username === 'kasir' && password === 'kasir000') {
      onLogin({ id: '2', username: 'kasir', role: 'kasir' });
    } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#37807c] px-4">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-2xl">
        <div className="text-center">
          <h2 className="mt-6 text-3xl font-extrabold text-slate-900">MITRA TOURS</h2>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              type="submit"
              className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-[#37807c] hover:bg-[#2d6865] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 transition-colors"
            >
              Login
            </button>
          </div>
